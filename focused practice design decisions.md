...
- Scoring uses miss rate plus slow-burst score against the user's language baseline, plus a char-affinity term: `score = confidence × (0.7·missRate + 0.3·slowScore) + 0.15·charAffinity`. Char affinity is the mean normalized substitution weight across an item's alpha chars (max-normalized over all target chars in the user's substitution data). Affinity bypasses the confidence gate so items composed of the user's known-bad keys can enter the pool even with low miss rate.
